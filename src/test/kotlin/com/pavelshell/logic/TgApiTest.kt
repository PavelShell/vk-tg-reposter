...
class TgApiTest {
    ...
    fun `send plain text without attachments`() {
        ...
        coEvery { bot.sendTextMessage(any<ChatId>(), publication.text!!) } returns buildResponse()
        publish(publication)

        coEvery { bot.sendTextMessage(chatIdEq(), publication.text!!) }
    }
...
//        every { bot.sendMessage(chatIdEq(), any<String>()) } returns buildResponse()
...
//            every { bot.sendMessage(chatIdEq(), publication.text!!) } returns buildResponse()
//            publish(publication)
//
//            verifySequence {
...
//    }
    ...
    private fun <T : MessageContent> buildResponse(sentMessageId: Long = 1L) = mockk<ContentMessage<T>>().also {
        every { it.messageId } returns MessageId(sentMessageId)
    }
...
//        Message(messageId = messageId, date = 1L, chat = Chat(1L, "Test"))
    ...
    private fun MockKMatcherScope.chatIdEq(chatId: Long = CHANNEL_ID) = match<ChatIdentifier> {
        it is ChatId && it.chatId?.long == chatId
    }
